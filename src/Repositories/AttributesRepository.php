...
class AttributesRepository extends RepositoryCore
{
  ...
  public function addNewAttributeGroupIfNotExists($attribute)
  {
    ...
    if ($exists) {
      $attribute->setId($exists['id_attribute_group']);
   ...
   * @param Product $product
   */
  public function updateAttributes(Product $product)
  {
    $this->updateProductAsAttribute($product);
    $this->updateOrDeleteProductAttributes($product);
  }

  /**
   * @param Product $product
   */
  public function updateProductAsAttribute(Product $product)
  {
//    $attribute = $this->getAttributeId("{$_ENV['MYSQL_PREFIX']}attribute_lang", $product->getFittingStandard());
//    $attributeId = $attribute['id_attribute'];
//    $productAttributeId = $this->getMaxIdFromTable("{$_ENV['MYSQL_PREFIX']}product_attribute", 'id_product_attribute') + 1;
...
//      'id_product_attribute' => $productAttributeId,
//    ];
    $this->update("{$_ENV['MYSQL_PREFIX']}product_attribute", $mlProductAttributeData, 'id_product_attribute', $product->getAttributeId());
//    $this->update("{$_ENV['MYSQL_PREFIX']}product_attribute_combination", $mlProductAttributeCombinationData);
    $this->update("{$_ENV['MYSQL_PREFIX']}product_attribute_shop", $mlProductAttributeShopData, 'id_product_attribute', $product->getAttributeId());
  }

  public function updateOrDeleteProductAttributes(Product $product)
  {


    $productAttributesInUse = $this->deleteExpendableAttributes($product);
   ...
   * @param Product $product
   */
  private function deleteExpendableAttributes(Product $product)
  {
    $productAttributesInUse = $this->search("{$_ENV['MYSQL_PREFIX']}product_attribute", ['id_product' => $product->getId()]);
    $productAttributes = $product->getAttributes();

    foreach ($productAttributesInUse as $key => $attributeInUse) {
      $contains = false;
      foreach ($productAttributes as $attribute) {
        if ($attribute->getAttributeId() === $attributeInUse['id_product_attribute']
          || $attributeInUse['id_product_attribute'] === $product->getAttributeId()) {
          $contains = true;
          break;
        }
      }
      if (!$contains) {
        $this->delete("{$_ENV['MYSQL_PREFIX']}product_attribute", ["id_product_attribute" => $attributeInUse->getId()]);
        unset($productAttributesInUse[$key]);
      }
    }
    return $productAttributesInUse;
  }
